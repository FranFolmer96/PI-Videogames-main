...
import { postVideogame, getGenders, getPlatforms, getVideogames } from '../../actions/actions'
...
const validate = (input) => {
   ...
   if(!input.genders.length) {
      errors.genders = 'genders must be completed'
   }
...
function Form() {
   ...
   const handleGendersDelete = (e) => {
      setInput({
         ...input,
         genders: input.genders.filter(g => g !== e)
      })
   }
      ...
      <div className="containerForm">
         ...
         <form className="form" onSubmit={e => handleSubmit(e)}>
            ...
            <div className="name formInput">
               ...
               <p>Genders:</p>
               { <select onChange={e => handleGenderSelect(e)}> {
                  genders.map(g => (
                     <option value={g.name}>{g.name}</option>
                  ))}
               </select> 
               }
               <div className="containerDivGenders">
                 {input.genders.map(e => 
               <div className="divGenders" value={e}>
         
                  <p>{e}</p>
                  <button key={e} className="buttonX" onClick={() => handleGendersDelete(e)}>X</button>
               </div>
            )}
